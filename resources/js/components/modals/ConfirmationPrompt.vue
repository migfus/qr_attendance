                        ...
                        >
                            ...
                            <div class="bg-white dark:bg-neutral-700 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                <div class="sm:flex sm:items-start">
                                    <!-- NOTE: ICON -->
                                    <div
                                        class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-brand-100 sm:mx-0 sm:h-10 sm:w-10"
                                    >
                                        <ExclamationTriangleIcon class="h-6 w-6 text-brand-600" aria-hidden="true" />
                                    </div>
                                    <!-- NOTE: TITLE -->
                                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                        <DialogTitle as="h3" class="text-base font-semibold leading-6 text-gray-900 dark:text-neutral-200">
                            ...
                            <div class="bg-gray-50 dark:bg-neutral-700 px-4 py-3 flex flex-col-reverse sm:flex-row gap-2 justify-end">
                                <AppButton v-if="!hideDoNotShowAgainButton" @click="doNotShowAgain()" :icon="TrashIcon">Do not show again.</AppButton>
                                <AppButton @click="$modal = false" :icon="XMarkIcon">Cancel</AppButton>
                                <AppButton @click="deleteUser()" color="brand" :icon="ArrowRightIcon">
...
import { ArrowRightIcon, TrashIcon, XMarkIcon } from '@heroicons/vue/24/outline'
...
const { id } = defineProps<{
    ...
    title: string
    id: string
    hideDoNotShowAgainButton?: boolean
}>()
...
function doNotShowAgain() {
    const confirm_storage_name = useLocalStorage(id, false)
    confirm_storage_name.value = true
    $modal.value = false
    $emit('confirm', true)
}
</script>
